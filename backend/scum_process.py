...
_METRICS_TTL_SEC = 30
_MAX_METRICS_CACHE_ITEMS = 1024
_MAX_LOGIN_COUNT_CACHE_ITEMS = 256
_LAST_CACHE_PRUNE_TS = 0.0


def _prune_cache_by_age(cache: Dict[str, Dict[str, Any]], now: float,
                        max_age_sec: float, max_items: int) -> None:
...
def _maybe_prune_runtime_caches(now: Optional[float] = None, *,
                                force: bool = False) -> None:
    """Keep short-lived metrics/login probes from growing forever."""
    global _LAST_CACHE_PRUNE_TS
    now = now or time.time()
    if not force and now - _LAST_CACHE_PRUNE_TS < 60:
        return
    _LAST_CACHE_PRUNE_TS = now
    _prune_cache_by_age(
        _METRICS_CACHE, now,
        max_age_sec=max(300, _METRICS_TTL_SEC * 10),
        max_items=_MAX_METRICS_CACHE_ITEMS,
    )
    _prune_cache_by_age(
        _LOGIN_COUNT_CACHE, now,
        max_age_sec=120,
        max_items=_MAX_LOGIN_COUNT_CACHE_ITEMS,
    )
    ...
    def _runner():
        ...
        try:
            ...
            for attempt in range(1, MAX_TRIES + 1):
                if REGISTRY[server_id]["install"].get("cancelled"):
                    log_lines.append("[LGSS] Installation abort requested...")
                    break
                if attempt > 1:
                ...
                proc = subprocess.Popen(
                    ...
                    # v1.0.36 PERFORMANCE FIX: SteamCMD writes its download
                    # progress on a SINGLE line, using carriage-return (\r)
                    # to overwrite the same column. Python's text-mode line
                    # iterator only yields on \n — which SteamCMD never
                    # sends until the very end of a phase. Result: the UI
                    # froze for tens of seconds then jumped 0% → 85%.
                    # We now read RAW BYTES and split on BOTH \n and \r,
                    # which gives us per-chunk progress updates ~10/sec.
                    bufsize=0,
                    creationflags=getattr(subprocess, "CREATE_NO_WINDOW", 0) if _is_windows() else 0,
                )
                
                with _LOCK:
                    REGISTRY[server_id]["install"]["proc"] = proc

                # Hand-rolled line reader that respects \r AND \n.
                buf = bytearray()
                while True:
                    if REGISTRY[server_id]["install"].get("cancelled"):
                        try:
                    ...
                    if not chunk:
                        # Process exited or pipe closed → drain any tail.
                        if buf:
                        ...
                        break
                    buf.extend(chunk)
                    # Split on every \r or \n. SteamCMD uses \r between
                    # progress updates and \n between phases — we treat
            ...
            is_cancelled = bool(REGISTRY[server_id]["install"].get("cancelled"))
            REGISTRY[server_id]["install"].update({
                ...
                "error": "Cancelled by user" if is_cancelled else (None if ok else f"SteamCMD exited with code {rc}"),
...
def wait_for_install(server_id: str, timeout_sec: float = 4 * 60 * 60,
                     poll_interval_sec: float = 2.0) -> Dict[str, Any]:
    """Block until a background SteamCMD install/update finishes."""
    deadline = time.time() + float(timeout_sec)
    while True:
        progress = get_install_progress(server_id)
        if not progress.get("running"):
            return progress
        if time.time() >= deadline:
            raise TimeoutError(f"Install/update timed out for server {server_id}")
        time.sleep(max(0.2, float(poll_interval_sec)))


def cleanup_server_state(server_id: str, folder_path: Optional[str] = None) -> None:
    """Drop in-memory process, install, and metrics state for a removed server."""
    with _LOCK:
        rec = REGISTRY.pop(server_id, {}) or {}
    pid = (rec.get("process") or {}).get("pid")
    if pid:
        _METRICS_CACHE.pop(f"_cpu_{pid}", None)
    _METRICS_CACHE.pop(f"ready_probe_{server_id}", None)
    _METRICS_CACHE.pop(f"a2s_{server_id}", None)
    if folder_path:
        _METRICS_CACHE.pop(f"disk_{folder_path}", None)
        _LOGIN_COUNT_CACHE.pop(f"login_online_{folder_path}", None)
    _maybe_prune_runtime_caches(force=True)


def abort_install(server_id: str) -> bool:
    """Abort a running installation / update process."""
    with _LOCK:
        inst = REGISTRY.get(server_id, {}).get("install")
        if not inst or not inst.get("running"):
            return False
        inst["cancelled"] = True
        inst["phase"] = "error"
        inst["error"] = "Cancelled by user"
        
        # Kill the subprocess if it's currently running
        proc = inst.get("proc")
        if proc:
            try:
                proc.kill()
        ...
        # Also sweep to kill any running steamcmd helper processes for safety
        if _is_windows():
            for img in ("steamcmd.exe", "steamservice.exe", "steamerrorreporter.exe"):
                try:
                    subprocess.run(
...
def check_master_server_reachable(timeout: float = 2.0) -> dict:
    ...
    try:
        ...
        try:
            ...
            # Fallback to standard Valve master server IP if DNS lookup fails
            addr = "208.64.200.117"
...
def start_server(server_id: str, folder_path: str, port: int = 7779,
    ...
    #
    # NETWORK / VISIBILITY flags — IMPORTANT LESSON LEARNED (v1.0.37g):
    # In v1.0.37e we stripped ALL port args because `-SteamServerPort=query_port`
    # was making SCUM advertise on the wrong socket. But removing `-QueryPort`
    # ENTIRELY caused a worse bug: Steam Dedicated Server API silently picks
    # the default Steam port (27015), which on Windows hosts running the Steam
    # *client* on the same machine is already taken → SteamGameServer_Init()
    # fails → server logs `Steam Dedicated Server API failed to initialize` →
    # all incoming Steam logins are kicked with `PreLogin failure:
    # incompatible_unique_net_id` (the NetDriver has no Steam UniqueNetId, so
    # it cannot accept clients that present a Steam ticket).
    #
    # The correct middle-ground (matches the SCUM-Server docs and what every
    # multi-server host on Discord runs):
    #   * `-port=N`        — game UDP socket
    #   * `-QueryPort=N+1` — Steam A2S + GameServer init port (UNIQUE per server)
    # We DO NOT pass `-SteamServerPort` (that was the v1.0.37e killer — it made
    # the engine advertise itself on the query port instead of the real connect
    # port). We DO NOT pass `-MULTIHOME` (binds the engine to a single NIC).
    # We DO NOT pass `-MaxPlayers` (read from ServerSettings.ini, the in-game
    # settings UI is the canonical source of truth).
    args = [
        str(exe),
        "-log",
        "-NoVerifyGC",
    ...
    # v1.0.37i — Firewall auto-configuration RESTORED (silent, no UI wizard).
    # User report: Windows Defender Firewall has 3 active profiles (Domain,
    # Private, Public) and all 3 block the SCUM server EXE + UDP ports by
    # default — players couldn't connect and the server didn't show in the
    # Steam browser. The v1.0.37f removal of this call was too aggressive;
    # the actual cause of admin frustration back then was the popup wizard,
    # not the firewall helper itself. Now we just call the idempotent helper
    # SILENTLY here — it covers all 3 profiles via `profile=any` and swallows
    # errors if the process isn't elevated (admin can still add rules later
    # via the Windows GUI, but at least we tried).
...
def get_metrics(server_id: str, folder_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    """
    now = time.time()
    _maybe_prune_runtime_caches(now)
