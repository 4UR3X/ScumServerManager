...
const STATUS_META = {
  ...
  Stopping:  { label: "server_status_stopping",  color: "var(--danger)" },
};
...
export const ServerDashboard = ({
  ...
  const { t } = useI18n();
  const [activeSection, setActiveSection] = useState(activeSectionProp || schema?.sections?.[0]?.key || "essentials");
  ...
  // v1.0.36 — Pending port changes accumulated from NetworkPortsPanel.
  // Persisted by the global Save Settings handler so the dedicated
  // "Save Ports" button could be removed (UX simplification).
  const [pendingPorts, setPendingPorts] = useState(null);
  // Same pattern for launch args — collected from LaunchArgsPanel callback,
  // persisted by the global Save Settings handler.
  const [pendingLaunchArgs, setPendingLaunchArgs] = useState(null);
  ...
  // Sync activeSection with prop changes
  useEffect(() => {
  ...
  const processAlive = isRunning || isStarting || server.status === "Stopping";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // v1.0.36 — If port changes are pending (NetworkPortsPanel callback),
      // persist them as part of the same "save" gesture.
      let finalServer = updated;
      if (pendingPorts) {
        try {
          finalServer = await endpoints.updateServerPorts(server.id, pendingPorts);
          setPendingPorts(null);
        } catch (e) {
          toast.error("Ports save failed: " + (e?.response?.data?.detail || e?.message));
        }
      }
      if (pendingLaunchArgs !== null) {
        try {
          finalServer = await endpoints.updateServerLaunchArgs(server.id, pendingLaunchArgs);
          setPendingLaunchArgs(null);
        } catch (e) {
          toast.error("Launch args save failed: " + (e?.response?.data?.detail || e?.message));
        }
      }
      onChange(finalServer);
      setDirty(false);
      try { localStorage.removeItem(draftKey); } catch (_) { /* private mode */ }
  ...
  const sections = useMemo(() => {
    const list = schema?.sections || [];
    if (activeSectionProp === "automation" || activeSectionProp === "discord") {
      return [];
    }
    return list.filter((sec) => sec.key !== "automation" && sec.key !== "discord");
  }, [schema, activeSectionProp]);
  ...
  const renderCategoryBody = (cat) => {
    ...
    switch (cat.renderer) {
      ...
      case "discord":
        // v1.0.37g — Discord bot is now per-server (each server has its own
        // token, status channel, and event-channel routing).
        return <DiscordBotSettings server={server} />;
      case "launch_args":
        return <LaunchArgsPanel
          server={server}
          onLaunchArgsChange={(args) => { setPendingLaunchArgs(args); setDirty(true); }}
        />;
      case "beta_settings":
        return (
          <BetaSettingsPanel
      ...
      default:
        return (
          <DynamicFields
            ...
            excludeFieldKeys={cat.excludeFieldKeys}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {server.status === "Stopping" && (
              <button
                ...
                disabled
                data-testid="server-stopping-indicator"
                style={{ opacity: 0.7 }}
              >
                <Icons.Square size={13} className="animate-pulse" /> {t("server_status_stopping")}...
              </button>
            )}
            {server.installed && (
              <button
              ...
              >
                ...
                {activeCat.key === "essentials_performance" && (
                  <NetworkPortsPanel
                    server={server}
                    onPortChange={(ports) => {
                      // v1.0.36 — Stash port changes in the draft so the
                      // global Save Settings button persists them via the
                      // ports endpoint alongside the INI categories.
