...
import { AlertTriangle, Network, Wifi, Search, Plug } from "lucide-react";
import { useI18n } from "../providers/I18nProvider";

// v1.0.36 — Removed unused imports (Save, endpoints, toast) after the
// inline Save button was deleted in favour of the global Save Settings flow.

/**
 * NetworkPortsPanel — SCUM 3-port layout.
 *
 * v1.0.36 CHANGE: This panel no longer has its own Save button. It's a
 * controlled component now — the parent (ServerDashboard) owns the values
 * and persists them via the global "Save Settings" button. Removed because
 * the user complained about button clutter ("o kadar save butonuna ne gerek
 * var"). The on-disk effect of changing ports requires a server restart
 * either way, so unifying the save action is a UX win.
 *
 ...
 */
export const NetworkPortsPanel = ({ server, onPortChange }) => {
  const { t } = useI18n();
  ...
  const handleGamePortChange = (e) => {
    ...
    let newQP = Number(queryPort);
    if (Number(queryPort) === oldGP + 1) {
      newQP = Number(v) + 1;
      setQueryPort(newQP);
    }
    onPortChange?.({ game_port: Number(v), query_port: newQP });
  };

  const handleQueryPortChange = (e) => {
    setQueryPort(e.target.value);
    onPortChange?.({ game_port: Number(gamePort), query_port: Number(e.target.value) });
  };
