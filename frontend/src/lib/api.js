...
export const endpoints = {
  ...
  // v1.0.45 — endpoints for Remote Hosted Server feature.
  listRemoteServers: () => api.get("/remote-servers").then(r => r.data),
  getRemoteServer: (id) => api.get(`/remote-servers/${id}`).then(r => r.data),
  createRemoteServer: (payload) => api.post("/remote-servers", payload).then(r => r.data),
  updateRemoteServer: (id, payload) => api.put(`/remote-servers/${id}`, payload).then(r => r.data),
  deleteRemoteServer: (id) => api.delete(`/remote-servers/${id}`).then(r => r.data),
  testRemoteConnection: (payload) => api.post("/remote-servers/test-connection", payload).then(r => r.data),
  testExistingRemote: (id) => api.post(`/remote-servers/${id}/test`).then(r => r.data),
  listRemoteDir: (id, path) => api.get(`/remote-servers/${id}/listing`, { params: path ? { path } : {} }).then(r => r.data),
  // v1.0.46 Phase 2 — remote settings INI read/write
  locateRemoteScum: (id) => api.post(`/remote-servers/${id}/locate-scum`).then(r => r.data),
  getRemoteSettings: (id) => api.get(`/remote-servers/${id}/settings`).then(r => r.data),
  updateRemoteSettings: (id, payload) => api.put(`/remote-servers/${id}/settings`, payload).then(r => r.data),

  saveConfig: (id) => api.post(`/servers/${id}/save-config`).then(r => r.data),
  // v1.0.37j — Quick traffic-light connection diagnostic (process+query+master)
  getConnectionDiagnostics: (id) => api.get(`/servers/${id}/diagnostics/connection`).then(r => r.data),
  ...
  // Discord bot (v1.0.37g — per-server)
  getServerDiscordBot: (id) => api.get(`/servers/${id}/discord/bot`).then(r => r.data),
  updateServerDiscordBot: (id, payload) => api.put(`/servers/${id}/discord/bot`, payload).then(r => r.data),
  getServerDiscordBotStatus: (id) => api.get(`/servers/${id}/discord/bot/status`).then(r => r.data),
  // System
  ...
  updateServerLaunchArgs: (id, launch_args) => api.put(`/servers/${id}/launch-args`, { launch_args }).then(r => r.data),
  openServerFolder: (id) => api.post(`/servers/${id}/open-folder`).then(r => r.data),
  ...
  abortInstall: (id) => api.post(`/servers/${id}/install/abort`).then(r => r.data),
};
