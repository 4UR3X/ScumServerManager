...
const Shell = () => {
  ...
  const [appVersion, setAppVersion] = useState({ current: "1.1.15", latest: "1.1.15", update_available: false });
  ...
  const load = useCallback(async () => {
    ...
    const [adminRes, setupRes, serverList, schemaRes, versionRes] = await Promise.all([
      ...
      safe(endpoints.getAppVersion(), { current: "1.1.15", latest: "1.1.15", update_available: false }),
    ]);
  ...
  const handleNavigate = (key) => {
    if (key === "configs" || key === "automation" || key === "discord") {
  ...
  return (
    <div className="h-full w-full flex bg-bg overflow-hidden">
      <Sidebar
        ...
        globalizationDir={setup?.globalization_dir}
      />
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          {(view === "configs" || view === "automation" || view === "discord") && (
            active ? (
              <ServerDashboard
                server={active}
                servers={servers}
                schema={schema}
                activeSectionProp={view === "configs" ? "essentials" : view}
                onChange={handleServerChange}
                onDelete={handleDelete}
          ...
          {view === "logs" && <LogsView servers={servers} activeServerId={activeId} onSelectServer={setActiveId} />}
          {view === "players" && <PlayersView servers={servers} activeServerId={activeId} onSelectServer={setActiveId} onServerUpdated={handleServerChange} />}
          {view === "backups" && <BackupsView servers={servers} activeServerId={activeId} onSelectServer={setActiveId} />}
          {view === "api" && <ApiView />}
        </div>
      </div>
